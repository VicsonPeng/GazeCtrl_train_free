...
behind her*, she turns fully away — camera sees only the back of her
head, no face at all — to look toward it; told the exact same screen
position is *close, in front of her*, she only tilts her head/eyes up
toward it, a smaller and less committed motion. Remove the depth image
entirely and Gemini doesn't move her at all — a 2D point alone doesn't
tell it whether "behind" is even a possibility.

Getting the "BEHIND" case to actually commit to that took three rounds of
prompt tuning, each one revealing the previous wasn't specific enough
about the geometry: "turn around like your name was called" only got a
head-and-shoulders check over the shoulder (still front-on to the camera);
"back should face the camera" got a genuine turn, but she still turned her
head sideways enough to show a face in profile. `build_depth_aware_prompt`'s
`BEHIND` case now says explicitly that the camera must see *only* the back
of the head — no face, cheek, or profile at all — which is what finally
produced a clean result. That's the line shipped in both
